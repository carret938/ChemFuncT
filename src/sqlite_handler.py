...
PathNoneType = Union[str, Path, None]
...
class SqliteHandler:
    ...
    def __init__(self):
        ...
        self._chem_func_uses_path = Path(__file__).resolve().parent / "data/ChemFUT.db"
        ...
        **kwargs
        ...
        for col in table_columns:
            if col not in kwargs.keys():
                raise ValueError(f"Column {col} exists in {table_name}, you "
                                 + "must provide this as an additional kw argument")
        
        ## create query 
        columns = ", ".join(table_columns)
        placeholders = ", ".join(["?"]*len(table_columns))
        values = tuple(kwargs[column] for column in table_columns) ## ensure values are in correct order
